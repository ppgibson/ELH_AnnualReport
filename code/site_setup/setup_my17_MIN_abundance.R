...
# Analysis options
  ...
  use.prv.strata <- TRUE   #Default = FALSE. Use standard, algorithm-based stratification (FALSE); or use BG's, previous stratification (TRUE).
  sub.sample <- FALSE        #Was there any subsampling at this site/year?
...
# Custom mods
  # Usable INT days
  INT.mods <- c("2017-03-31", "2017-05-12")  #Vector of INT dates determined, based on subjective assessment, to be usable for mark-recap anl.

  # Adjustments to stratification
  strat.mods <- list()  #No custom mods applied, beyond using BG's prv stratification. Maybe that shd be recorded here.
